...
export function createWebClient(token?: string) {
  const apiUrl = process.env.NEXT_PUBLIC_SORAKU_URL ?? "";
...
export function createBotClient() {
  const apiUrl = process.env.SORAKU_URL ?? "";
...
export function createMobileClient(token?: string) {
  const apiUrl = process.env.EXPO_PUBLIC_SORAKU_URL ?? "";
  return createApiClient({
    baseUrl: apiUrl,
    token,
  });
...
export function createInternalClient(secret?: string) {
  ...
  return createApiClient({
    baseUrl: apiUrl,
    internalSecret: secret ?? process.env.SORAKU_SECRET,
  });
}
