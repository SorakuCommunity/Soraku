'use client'
// components/Navbar.tsx — SORAKU v1.0.a3.5
// Fixed: /Admin casing, AGENSI in admin roles, username display
...
// All 4 roles that can access /Admin
  ...
  useEffect(() => {
    supabase.auth.getUser().then(({ data }) => {
      ...
      if (data.user) {
        supabase
          .from('users')
          .select('role, theme_mode, username')
          .eq('id', data.user.id)
          .single()
          .then(({ data: p }) => {
            if (p) {
              setRole(p.role)
              setThemeMode((p.theme_mode as ThemeMode) ?? 'dark')
    ...
    const { data: sub } = supabase.auth.onAuthStateChange((_e, session) => {
      ...
      if (!session?.user) { setRole(null); setUsername(null) }
    })
        ...
        <div className="hidden md:flex items-center gap-1">
          {NAV_LINKS.map(l => (
            <Link key={l.href} href={l.href}
              ...
              style={{ color: active(l.href) ? 'var(--color-primary)' : 'var(--text-sub)',
                       backgroundColor: active(l.href) ? 'var(--hover-bg)' : 'transparent' }}>
              {l.label}
            </Link>
          ))}
        </div>
        ...
        <div className="flex items-center gap-2">
          {user ? (
              ...
              {userOpen && (
                <div className="absolute right-0 top-full mt-1 w-56 rounded-xl border shadow-xl py-1 z-50"
                  ...
                  <Link href="/profile" onClick={() => setUserOpen(false)}
                    className="flex items-center gap-2 px-3 py-2.5 text-sm transition-colors hover:bg-[var(--hover-bg)]"
                    style={{ color: 'var(--text-sub)' }}>
                    <User size={14} /> Profil Saya
                  </Link>

                  {/* Admin Panel — semua 4 role admin */}
                  {isAdmin && (
                  ...
                  <div className="px-3 pt-2 pb-1 border-t" style={{ borderColor: 'var(--border)' }}>
                    ...
                    <div className="grid grid-cols-3 gap-1">
                      {([ ['dark','Gelap',Moon], ['light','Terang',Sun], ['auto','Otomatis',Monitor] ] as const).map(([m, label, Icon]) => (
                        <button key={m} onClick={() => handleTheme(m)}
                  ...
                  <button onClick={signOut}
                    className="flex items-center gap-2 px-3 py-2.5 text-sm w-full transition-colors hover:bg-[var(--hover-bg)]"
                    style={{ color: '#ef4444' }}>
            ...
            <div className="hidden md:flex items-center gap-2">
              <Link href="/login" className="px-4 py-2 rounded-lg text-sm font-medium min-h-[44px] flex items-center"
                style={{ color: 'var(--text-sub)' }}>Masuk</Link>
              <Link href="/register" className="px-4 py-2 rounded-lg text-sm font-semibold min-h-[44px] flex items-center"
                style={{ backgroundColor: 'var(--color-primary)', color: '#fff' }}>Daftar</Link>
            </div>
          )}
          ...
          {!user ? (
            <div className="grid grid-cols-2 gap-2 pt-2">
              <Link href="/login" onClick={() => setOpen(false)}
                ...
                style={{ color:'var(--text-sub)', borderColor:'var(--border)' }}>Masuk</Link>
            ...
            <button onClick={signOut}
              className="flex items-center gap-2 w-full px-3 py-3 rounded-lg text-sm"
              style={{ color:'#ef4444' }}>
              <LogOut size={14} /> Keluar
            </button>
          )}
