...
import {
  ...
  LogOut, LayoutDashboard, Sparkles,
} from 'lucide-react'
...
function UserAvatar({ url, name }: { url: string | null; name: string | null }) {
  if (url) {
    return (
      ...
      <img src={url} alt={name ?? 'avatar'} className="w-7 h-7 rounded-full" />
    )
  }
  return (
  ...
  return (
    <nav
      className={cn(
        ...
        scrolled
          ? 'bg-soraku-dark/95 backdrop-blur-md border-b border-soraku-border shadow-lg'
          : 'bg-transparent',
      )}
    >
          ...
          <div className="flex items-center gap-3">
            {authed ? (
              <div className="relative">
                ...
                >
                  <UserAvatar url={user?.avatar_url ?? null} name={user?.display_name ?? null} />
                ...
                <AnimatePresence>
                  {dropOpen && (
                    <motion.div
                      ...
                      className="absolute right-0 mt-2 w-52 glass rounded-xl shadow-2xl overflow-hidden"
                    >
                      {user?.username && (
                        <Link
                      ...
                      <button
                        ...
                        className="flex items-center gap-3 px-4 py-3 hover:bg-red-900/20 transition-colors w-full text-left"
                      >
            ...
            <button
              ...
              className="md:hidden text-soraku-sub hover:text-soraku-text"
              ...
              <div className="py-3 space-y-1">
                {NAV_LINKS.map(l => (
                  <Link
                    ...
                    className="block px-4 py-2 text-soraku-sub hover:text-soraku-text hover:bg-white/5 rounded-lg transition-colors"
                  >
                    {l.label}
                  </Link>
                ))}
