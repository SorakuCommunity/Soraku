...
import { Twitter, Instagram, Youtube, Twitch, Globe, Music, ExternalLink, Heart, ShoppingBag } from 'lucide-react'
import type { User } from '@/types'

interface Props { params: Promise<{ username: string }> }

const SLink = ({ href, label, Icon }: { href: string; label: string; Icon: React.ElementType }) => (
  <a href={href} target="_blank" rel="noopener noreferrer"
    ...
    <Icon className="w-4 h-4 text-soraku-primary shrink-0" /><span className="flex-1">{label}</span>
    ...
    <div className="min-h-screen pt-24 pb-16 px-4">
      <div className="max-w-2xl mx-auto">
        ...
        <div className="glass rounded-3xl p-8 mb-6 text-center">
          {user.avatar_url
            ? <img src={user.avatar_url} alt={user.display_name ?? ''} className="w-24 h-24 rounded-full mx-auto mb-4 border-4 border-purple-500/30" />
          ...
          <span className={`inline-block text-xs font-semibold px-3 py-1 rounded-full ${getRoleBadgeColor(user.role)}`}>{user.role}</span>
          {user.bio && <p className="text-soraku-sub text-sm mt-4 leading-relaxed max-w-xs mx-auto">{user.bio}</p>}
        </div>

        {socials.length > 0 && (
          <div className="glass rounded-2xl p-6 mb-6">
            <h2 className="text-xs font-semibold uppercase tracking-wider text-soraku-sub mb-4">Media Sosial</h2>
            <div className="grid sm:grid-cols-2 gap-3">{socials.map(l => <SLink key={l.href} {...l} />)}</div>
          </div>
        )}

        {fans.length > 0 && (
          <div className="glass rounded-2xl p-6">
            <h2 className="text-xs font-semibold uppercase tracking-wider text-soraku-sub mb-4">Fan Zone & Support</h2>
            <div className="grid sm:grid-cols-2 gap-3">{fans.map(l => <SLink key={l.href} {...l} />)}</div>
          </div>
        )}
      </div>
    </div>
