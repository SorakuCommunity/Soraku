  ...
  return (
    <div className="min-h-screen pt-24 pb-16 px-4">
      <div className="max-w-3xl mx-auto">
        <Link href="/komunitas" className="inline-flex items-center gap-2 text-soraku-sub hover:text-soraku-primary mb-8 transition-colors text-sm">
        ...
        <div className="glass rounded-2xl p-8 mb-6">
          ...
          <div className="flex items-center gap-3 mb-6 pb-6 border-b border-soraku-border">
            <img src={d.author.avatarUrl} alt={d.author.login} className="w-8 h-8 rounded-full" />
            <div>
            ...
            <div className="ml-auto flex items-center gap-1 text-soraku-sub text-sm">
              <ThumbsUp className="w-4 h-4" /> {d.upvoteCount}
            </div>
          </div>
          <div className="text-soraku-sub leading-relaxed whitespace-pre-wrap text-sm">{d.body}</div>
        </div>

        <h2 className="flex items-center gap-2 font-semibold text-lg mb-4">
          <MessageSquare className="w-5 h-5 text-soraku-primary" />{d.comments.totalCount} Komentar
        </h2>
        <div className="space-y-4">
          {d.comments.nodes.map(c => (
            <div key={c.id} className="glass rounded-xl p-5">
              <div className="flex items-center gap-3 mb-3">
                <img src={c.author.avatarUrl} alt={c.author.login} className="w-7 h-7 rounded-full" />
                <div>
                ...
                <div className="ml-auto flex items-center gap-1 text-soraku-sub text-xs">
                  <ThumbsUp className="w-3 h-3" /> {c.upvoteCount}
                </div>
              </div>
              <p className="text-soraku-sub text-sm leading-relaxed whitespace-pre-wrap">{c.body}</p>
            </div>
          ))}
        </div>
      </div>
