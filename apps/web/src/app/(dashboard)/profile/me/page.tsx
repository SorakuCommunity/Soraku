...
function Field({
  ...
  return (
    <div className="space-y-1.5">
      <label className="text-muted-foreground/60 text-xs font-semibold">{label}</label>
      {children}
      {hint && <p className="text-muted-foreground/35 text-[10px]">{hint}</p>}
    </div>
  )
}
...
function Input({
  ...
  return (
    <input
      ...
      disabled={disabled}
      className={cn(
        'border-border/25 w-full rounded-xl border bg-transparent px-3.5 py-2.5 text-sm outline-none',
...
export default function PrivateProfilePage() {
  ...
  if (loading)
    return (
      <div className="mx-auto max-w-2xl animate-pulse space-y-6 px-4 pt-16 pb-24">
        <div className="bg-muted/20 h-3 w-24 rounded" />
        <div className="bg-muted/15 h-40 rounded-3xl" />
        <div className="flex items-center gap-4">
          <div className="bg-muted/20 h-20 w-20 rounded-2xl" />
          <div className="flex-1 space-y-2">
            <div className="bg-muted/20 h-5 w-40 rounded" />
  ...
  return (
    <div className="mx-auto max-w-2xl px-4 pt-8 pb-24">
      ...
      {toast && (
        <div
          className={cn(
            'fixed bottom-6 left-1/2 z-50 flex -translate-x-1/2 items-center gap-2.5 rounded-2xl px-5 py-3 text-sm font-semibold shadow-xl transition-all',
            toast.type === 'ok'
      ...
      <div className="mb-6 flex items-center justify-between">
        <div>
          <p className="text-muted-foreground/30 mb-1 text-[9px] font-black tracking-[0.25em] uppercase">
            Dashboard
          </p>
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            onClick={logout}
            className="border-border/25 text-muted-foreground/50 flex items-center gap-1.5 rounded-xl border px-3.5 py-2 text-xs font-semibold transition-all hover:border-red-400/30 hover:text-red-400"
          >
      ...
      <div className="from-primary/20 via-border/20 mb-8 h-px bg-gradient-to-r to-transparent" />

      {/* ── PREVIEW CARD ── */}
      <div className="from-primary/10 via-primary/4 to-accent/8 relative mb-8 overflow-hidden rounded-3xl bg-gradient-to-br">
        {/* Cover */}
        <div
        ...
        >
          ...
          <div className="absolute inset-0 flex items-center justify-center bg-black/0 transition-colors group-hover:bg-black/20">
            <Camera className="h-5 w-5 text-white opacity-0 transition-opacity group-hover:opacity-70" />
          </div>
        ...
        <div className="px-5 pb-5">
          {/* Avatar */}
          <div
            className="group relative -mt-11 h-[88px] w-[88px] cursor-pointer"
            onClick={() => {
              const url = prompt('URL avatar baru:', avatarurl)
              if (url !== null) {
                handleImgUrl('avatar', url)
              }
            }}
          >
            ...
            <div className="border-background bg-card absolute inset-[4px] overflow-hidden rounded-2xl border-[3px] shadow-xl">
              ...
              <div className="absolute inset-0 flex items-center justify-center bg-black/0 transition-colors group-hover:bg-black/30">
                <Pencil className="h-4 w-4 text-white opacity-0 transition-opacity group-hover:opacity-80" />
              </div>
            </div>
            <div
          ...
          <p className="text-muted-foreground/40 mt-0.5 text-xs">
            @{username || profile.username} · {lvlTitle}
          </p>
          ...
          <div className="mt-4 flex items-center gap-5">
            {[
              ...
              <div key={i} className="text-center">
                ...
                <div className="text-muted-foreground/35 text-[9px] tracking-wider uppercase">
                  {s.label}
                </div>
              </div>
            ))}
          ...
          {/* XP bar */}
          <div className="mt-4">
            <div className="bg-muted/15 h-1 w-full overflow-hidden rounded-full">
              <div
                className="h-full rounded-full"
                style={{
                  width: `${xpPct}%`,
                  background: `linear-gradient(to right, ${rm.color}60, ${rm.color})`,
                }}
              />
            </div>
            <p className="text-muted-foreground/25 mt-1 text-right text-[9px]">
              {xpPct}% menuju level {lvl.level + 1}
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="space-y-7">
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
              ...
              <Field label="Username" hint="Huruf kecil, angka, underscore">
                <div className="relative">
                  <span className="text-muted-foreground/30 absolute top-1/2 left-3.5 -translate-y-1/2 text-sm">
                    @
                  </span>
                  <input
                    value={username}
                    ...
                    className="border-border/25 placeholder:text-muted-foreground/25 focus:border-primary/40 w-full rounded-xl border bg-transparent py-2.5 pr-3.5 pl-8 font-mono text-sm transition-colors outline-none"
            ...
            <Field label="Bio" hint="Maksimal 300 karakter">
              <textarea
                ...
                className="border-border/25 placeholder:text-muted-foreground/25 focus:border-primary/40 w-full resize-none rounded-xl border bg-transparent px-3.5 py-2.5 text-sm transition-colors outline-none"
              />
              <p className="text-muted-foreground/25 text-right text-[10px]">{bio.length}/300</p>
            </Field>
          </div>
        </div>

        <div className="from-border/20 h-px bg-gradient-to-r to-transparent" />
          ...
          <div className="space-y-3">
            {SOCIAL_FIELDS.map(({ key, label, placeholder, Icon }) => (
              <div key={key} className="flex items-center gap-3">
                <div className="border-border/20 text-muted-foreground/40 flex h-9 w-9 flex-shrink-0 items-center justify-center rounded-xl border">
                ...
                <input
                  ...
                  placeholder={placeholder}
                  className="border-border/20 placeholder:text-muted-foreground/20 focus:border-primary/35 flex-1 rounded-xl border bg-transparent px-3.5 py-2 text-sm transition-colors outline-none"
                />
                {socials[key] && (
                  <a
                    href={SOCIAL_CONFIG.find((s) => s.key === key)?.getHref(socials[key]) ?? '#'}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="border-border/20 text-muted-foreground/30 hover:text-foreground hover:border-border/50 flex h-9 w-9 flex-shrink-0 items-center justify-center rounded-xl border transition-all"
                  >
        ...
        <div>
          ...
          <button
            ...
            className={cn(
              'flex w-full items-center gap-3 rounded-2xl border px-5 py-3.5 text-left transition-all',
          ...
          >
            ...
            <div>
              ...
              <p className="text-muted-foreground/40 mt-0.5 text-xs">
                Sembunyikan detail profilmu dari publik
              </p>
            </div>
          </button>
        </div>
        ...
        {profile.badges.length > 0 && (
          <>
            <div className="from-border/20 h-px bg-gradient-to-r to-transparent" />
            <div>
              <p className="text-muted-foreground/30 mb-3 flex items-center gap-1.5 text-[9px] font-black tracking-[0.25em] uppercase">
        ...
        <div className="sticky bottom-6 pt-4">
          <button
            ...
            className={cn(
              ...
              dirty && !saving
                ? 'bg-primary shadow-primary/20 text-white shadow-xl hover:-translate-y-0.5'
