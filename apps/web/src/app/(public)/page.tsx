...
const CATEGORIES = [
  ...
  { label: 'Belajar Bareng', color: '#a78bfa', glow: 'rgba(167,139,250,0.5)' },
  { label: 'VTuber', color: '#f472b6', glow: 'rgba(244,114,182,0.5)' },
  { label: 'Fanart', color: '#34d399', glow: 'rgba(52,211,153,0.5)' },
  { label: 'Web Development', color: '#fbbf24', glow: 'rgba(251,191,36,0.5)' },
  { label: 'Game Development', color: '#E8C2A8', glow: 'rgba(232,194,168,0.5)' },
  { label: 'Kreator', color: '#818cf8', glow: 'rgba(129,140,248,0.5)' },
  { label: 'Komunitas', color: '#6ee7b7', glow: 'rgba(110,231,183,0.5)' },
  { label: 'Design Graphics', color: '#fb923c', glow: 'rgba(251,146,60,0.5)' },
  { label: 'Design UI/UX', color: '#e879f9', glow: 'rgba(232,121,249,0.5)' },
  { label: 'Belajar', color: '#f87171', glow: 'rgba(248,113,113,0.5)' },
  { label: 'Berkembang', color: '#9ca3af', glow: 'rgba(156,163,175,0.5)' },
  { label: 'Have Fun', color: '#fbbf24', glow: 'rgba(251,191,36,0.5)' },
  { label: 'Tech Talk', color: '#38bdf8', glow: 'rgba(56,189,248,0.5)' },
  { label: 'DevOps', color: '#f472b6', glow: 'rgba(244,114,182,0.5)' },
  { label: 'Mobile Development', color: '#3b82f6', glow: 'rgba(59,130,246,0.5)' },
]
    ...
    <section className="relative overflow-hidden">
      ...
      <div className="relative flex min-h-[95svh] flex-col justify-center px-6 pt-24 pb-14 lg:hidden">
        <div className="relative z-10">
          <span
            className="mb-3 block text-[clamp(1.2rem,4vw,1.5rem)] tracking-wider"
            style={{ fontFamily: "var(--font-script,'Style Script',cursive)", color: `${AC}B0` }}
          >
            Temukan komunitas kreatifmu
          </span>
          <div className="mb-5 inline-flex items-center gap-2 rounded-full border border-white/[0.08] bg-white/[0.03] px-3.5 py-1.5">
           ...
           <h1 className="mb-4 text-[clamp(3.2rem,10vw,4.8rem)] font-[Playfair_Display] font-bold leading-[1.1] tracking-tighter text-white/95">
             Temukan Duniamu di{' '}
             <span
               className="bg-clip-text text-transparent"
               style={{
                 backgroundImage: `linear-gradient(130deg,${P} 0%,#90c8e8 40%,${AC} 75%,#d4a882 100%)`,
      ...
      <div className="relative hidden min-h-[95vh] items-center lg:grid lg:grid-cols-[1fr_500px] xl:grid-cols-[1fr_560px]">
        <div className="relative z-10 px-10 py-20 xl:px-16">
          <span
            className="mb-4 block text-[clamp(1.2rem,4vw,1.5rem)] tracking-wider"
            style={{ fontFamily: "var(--font-script,'Style Script',cursive)", color: `${AC}B0` }}
          >
            Temukan komunitas kreatifmu
          </span>
        ...
        <div className="relative h-full min-h-[95vh]">
          ...
          {[
            { text: '💬 Diskusi', top: '44%', left: '1rem', delay: '3s' },
            { text: '🌱 Berkembang', top: '23%', left: '1.5rem', delay: '1s' },
            { text: '✨ Belajar', top: '58%', left: '1.5rem', delay: '2s' },
          ].map((b, i) => (
            <div
              key={i}
...
function SH({ eyebrow, title, href }: { eyebrow: string; title: string; href?: string }) {
  return (
    <div className="mb-6 flex items-end justify-between sm:mb-8">
      <div>
        <p className="mb-1.5 text-[9px] font-black tracking-[0.3em] uppercase" style={{ color: P + '80' }}>
          {eyebrow}
        </p>
        <h2 className="text-xl font-black tracking-tight sm:text-2xl lg:text-3xl" style={{ color: L }}>
          {title}
        </h2>
      </div>
      {href && (
        <Link href={href} className="group flex items-center gap-1 text-xs font-bold transition-colors" style={{ color: S + '80' }}>
          Lihat Semua{' '}
          <ChevronRight className="h-3.5 w-3.5 transition-transform group-hover:translate-x-0.5" style={{ color: P }} />
        </Link>
      )}
    </div>
  )
...
// ─── Glass Card ───────────────────────────────────────────────────────────────
function GlassCard({ children, className, href }: { children: React.ReactNode; className?: string; href?: string }) {
  const base = cn(
  ...
  if (href)
    return (
      <Link href={href} className={cn(base, 'hover:-translate-y-1 hover:border-white/[0.12] hover:shadow-xl hover:shadow-black/30')}>
        {children}
      </Link>
    )
...
function GridSkeleton({ cols = 6 }: { cols?: number }) {
  return (
    <div className={`grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-4 ${cols === 6 ? 'xl:grid-cols-6' : ''}`}>
    ...
    <GlassCard href={`/events/${event.slug}`}>
      <div className="relative aspect-[3/4] overflow-hidden rounded-2xl">
        {event.coverurl ? (
          ...
          <Image src="/assets/brand/logo.png" alt="Event cover" fill className="object-cover" unoptimized sizes="(max-width:640px)50vw,(max-width:1024px)33vw,16vw" />
        )}
        <div className="absolute inset-0 bg-gradient-to-t from-black/85 via-black/20 to-transparent" />
        <span className={cn('absolute top-2.5 right-2.5 flex items-center gap-1.5 rounded-full border px-2 py-1 text-[9px] font-black backdrop-blur-md', st.cls)}>
    ...
    <GlassCard href={`/blog/${blog.slug}`}>
      <div className="relative aspect-[3/4] overflow-hidden rounded-2xl">
        {blog.coverurl ? (
          ...
          <Image src="/assets/brand/logo.png" alt="Blog cover" fill className="object-cover" unoptimized sizes="(max-width:640px)50vw,(max-width:1024px)33vw,16vw" />
        )}
  ...
  return (
    <GlassCard href="/gallery" className="group">
      <div className={cn('relative overflow-hidden rounded-2xl', heights[idx % heights.length])}>
        {item.imageurl ? (
          <Image src={item.imageurl} alt={item.title ?? ''} fill className="object-cover transition-transform duration-500 group-hover:scale-105" unoptimized sizes="(max-width:640px)50vw,25vw" />
        ) : (
    ...
    <GlassCard className="shadow-2xl shadow-black/40 backdrop-blur-xl">
      ...
      <div className="flex items-center justify-between gap-3 border-b border-white/[0.06] px-5 py-4">
        <div className="flex items-center gap-3">
          <div className="relative h-10 w-10 flex-shrink-0 overflow-hidden rounded-full border border-white/15">
            <Image src="/assets/brand/logo.png" alt="S" width={40} height={40} className="object-cover" />
          </div>
          <div>
            <p className="text-sm font-black" style={{ color: L }}>{discord.name}</p>
            <p className="text-[10px]" style={{ color: S + '80' }}>Server Discord Resmi</p>
          </div>
        </div>
        <a href="https://discord.gg/qm3XJvRa6B" target="_blank" rel="noopener noreferrer" className="flex items-center gap-1.5 rounded-xl bg-indigo-500 px-3.5 py-2 text-xs font-bold text-white transition-colors hover:bg-indigo-400">
      ...
      <div className="grid grid-cols-2 divide-x divide-white/[0.06] border-b border-white/[0.06]">
        <div className="flex flex-col items-center gap-1 py-4">
          ...
          <span className="text-[10px]" style={{ color: S + '70' }}>Member Online</span>
        </div>
        <div className="flex flex-col items-center gap-1 py-4">
          <div className="flex items-center gap-2">
            <Users className="h-5 w-5" style={{ color: S + '60' }} />
            <span className="text-2xl font-black" style={{ color: L }}>500+</span>
          </div>
          <span className="text-[10px]" style={{ color: S + '70' }}>Total Member</span>
        </div>
      </div>
      <div className="px-5 py-4">
        <p className="mb-3 text-[9px] font-black tracking-[0.2em] uppercase" style={{ color: S + '50' }}>Sedang Online</p>
        {discord.loading ? (
          <div className="grid grid-cols-2 gap-2">
            {[1, 2, 3, 4].map((i) => (
              <div key={i} className="flex animate-pulse items-center gap-2 rounded-xl bg-white/[0.03] px-3 py-2">
          ...
          <p className="py-4 text-center text-sm" style={{ color: S + '40' }}>Widget tidak tersedia</p>
        ) : (
          <div className="grid grid-cols-2 gap-2">
            {discord.members.map((m, i) => (
              <div key={i} className="flex items-center gap-2 rounded-xl border border-white/[0.05] bg-white/[0.03] px-3 py-2">
                <div className="relative flex-shrink-0">
                  {m.avatar ? (
                    <div className="h-7 w-7 overflow-hidden rounded-full border border-white/10">
                      <Image src={m.avatar} alt="" width={28} height={28} className="object-cover" />
                    </div>
                  ) : (
                  ...
                  <span className={cn('absolute -right-0.5 -bottom-0.5 h-2.5 w-2.5 rounded-full border-[1.5px] border-[#1C1E22]', SC[m.status] ?? SC.online)} />
                </div>
                <div className="min-w-0 flex-1">
                  <p className="truncate text-xs font-semibold" style={{ color: L + 'B0' }}>{m.username}</p>
                  {m.activity && <p className="truncate text-[9px]" style={{ color: S + '50' }}>{m.activity}</p>}
                </div>
              </div>
            ))}
          </div>
      ...
      <div className="px-5 pb-5">
        <a href="https://discord.gg/qm3XJvRa6B" target="_blank" rel="noopener noreferrer" className="flex w-full items-center justify-center gap-2 rounded-xl border border-indigo-500/20 bg-indigo-500/8 py-2.5 text-xs font-semibold text-indigo-300/70 transition-colors hover:bg-indigo-500/15 hover:text-indigo-300">
...
export default function HomePage() {
  ...
  // Use empty arrays when API returns empty (no dummy data)
  const displayEvents = data.events
  const displayBlogs = data.blogs
