...
export const metadata: Metadata = {
  title: 'Agensi | Soraku',
  description: 'Talent management Soraku. VTuber, kreator, cosplayer, dan musisi Indonesia.',
}
...
function VtuberCard({ v }: { v: VtuberRow }) {
  ...
  return (
    <div className="group overflow-hidden rounded-md border-2 border-black bg-surface shadow-[4px_4px_0px_#000] transition-all hover:translate-x-[2px] hover:translate-y-[2px] hover:shadow-[2px_2px_0px_#000]">
      <div className="relative h-28 bg-gradient-to-br from-primary/20 to-violet-500/20">
        ...
        <div className="absolute top-3 right-3">
          <span className="rounded-sm border border-black bg-primary/20 px-2 py-0.5 text-[10px] font-bold text-primary">
            VTuber
          </span>
        </div>
        {v.islive && (
      ...
      <div className="p-4 pt-10">
        <h3 className="text-sm font-bold text-foreground">{v.charactername ?? v.name}</h3>
        {v.charactername && (
        ...
        <div className="mt-3 flex items-center gap-2">
          ...
          {socials.instagram && (
            <a href={socials.instagram} target="_blank" rel="noopener noreferrer" className="text-muted hover:text-pink-500">
    ...
    <div className="mx-auto max-w-7xl px-4 py-8">
      <div className="mb-12 text-center">
        ...
        <p className="mx-auto mt-4 max-w-lg text-sm text-muted">
          Kenalan dengan talent virtual dari Soraku, kreator yang menghibur dan
          menginspirasi.
        </p>
      </div>
      ...
      {(vtubers ?? []).length === 0 && (
        <div className="flex flex-col items-center justify-center py-20">
          ...
          <p className="text-sm text-muted">Belum ada talent.</p>
        </div>
      )}
    </div>
  )
