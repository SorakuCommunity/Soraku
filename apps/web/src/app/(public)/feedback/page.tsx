...
export default function FeedbackPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        setResult({ type: 'success', msg: 'Feedback berhasil dikirim! Kami akan segera merespons.' })
    ...
    <div className="mx-auto max-w-2xl px-4 py-16 sm:px-6">
      <div className="mb-10 text-center">
        ...
        <p className="text-muted-foreground/70 mx-auto mt-4 max-w-md text-sm leading-relaxed">
          ...
          say hi? Isi formulir di bawah dan kami akan merespons secepatnya.
        </p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        <div className="grid gap-5 sm:grid-cols-2">
          <div>
            ...
            <input
              ...
              className="border-border/40 bg-white/[0.03] focus:border-primary/40 w-full rounded-xl border px-4 py-2.5 text-sm outline-none transition-colors placeholder:text-muted-foreground/30"
            />
          </div>
          <div>
        ...
        <div>
          ...
          <textarea
            ...
            className="border-border/40 bg-white/[0.03] focus:border-primary/40 w-full resize-none rounded-xl border px-4 py-3 text-sm outline-none transition-colors placeholder:text-muted-foreground/30"
          />
          <p className="text-muted-foreground/30 mt-1 text-right text-[10px]">{form.message.length}/2000</p>
        </div>
