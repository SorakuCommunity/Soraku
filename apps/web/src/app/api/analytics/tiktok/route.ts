...
export async function POST(req: NextRequest) {
  if (!ACCESS_TOKEN) {
    return NextResponse.json({ ok: false, error: 'TIKTOK_ACCESS_TOKEN not configured' }, { status: 500 })
  }

  try {
    const body: EventBody = await req.json()
    if (!body.event) return NextResponse.json({ ok: false, error: 'event required' }, { status: 400 })
    ...
    const eventId   = body.event_id ?? `${body.event}-${now}-${Math.random().toString(36).slice(2, 8)}`
