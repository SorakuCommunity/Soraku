...
const PostSchema = z.object({
  ...
  coverurl:    z.string().url().optional().or(z.literal('')).transform(v => v || undefined),
...
async function notifyDiscordBlog(post: {
  ...
  const tagsStr = (post.tags ?? []).length > 0
...
export async function GET(req: NextRequest) {
  try {
    ...
    const { data, error } = await adminDb()
      .from('posts')
      .select('id,slug,title,excerpt,coverurl,ispublished,tags,createdat,publishedat,authorid,viewcount,likecount')
