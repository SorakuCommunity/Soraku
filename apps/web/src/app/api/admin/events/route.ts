...
async function sendDiscordEventEmbed(event: {
  ...
  fields.push({
    name: '🌐 Tipe',
    value: event.isonline ? 'Online' : `Offline${event.location ? ` — ${event.location}` : ''}`,
    inline: true,
  })
  fields.push({
    name: '💰 Biaya',
...
export async function GET(req: NextRequest) {
  try {
    ...
    const { data, error } = await adminDb()
      .from('events')
      .select(
        'id,slug,title,startdate,enddate,isonline,ispublished,tags,registrationurl,gametype,ispaid,price,paymentmethods,registrationopen,createdat'
      )
      ...
      if (env.BOT_WEBHOOK_URL && env.BOT_WEBHOOK_SECRET) {
        fetch(`${env.BOT_WEBHOOK_URL}/announce/event`, {
          ...
          headers: {
            ...
            'x-webhook-secret': env.BOT_WEBHOOK_SECRET,
          },
          body: JSON.stringify({
