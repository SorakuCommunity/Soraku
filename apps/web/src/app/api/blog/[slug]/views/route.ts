...
export async function POST(
  ...
  try {
    ...
    const { data: post } = await adminDb()
      .from('posts').select('id,viewcount').eq('slug', slug).eq('ispublished', true).maybeSingle()
...
export async function GET(
  ...
  try {
    const { slug } = await params
    const { data } = await adminDb()
      .from('posts').select('viewcount').eq('slug', slug).eq('ispublished', true).maybeSingle()
    return ok({ viewcount: data?.viewcount ?? 0 })
  } catch { return SERVER_ERROR() }
}
