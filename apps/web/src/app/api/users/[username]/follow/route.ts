...
export async function GET(
  ...
  const [fersRes, fingsRes] = await Promise.all([
    adminDb().from('follows').select('id', { count: 'exact', head: true }).eq('followingid', target.id),
    adminDb().from('follows').select('id', { count: 'exact', head: true }).eq('followerid',  target.id),
  ])

  let isFollowing = false
  if (session?.id) {
    const { data } = await adminDb().from('follows')
      .select('id').eq('followerid', session.id).eq('followingid', target.id).maybeSingle()
    isFollowing = !!data
  }
...
export async function POST(
  ...
  const { data: existing } = await adminDb().from('follows')
    .select('id').eq('followerid', session.id).eq('followingid', target.id).maybeSingle()

  if (existing) {
    await adminDb().from('follows').delete()
      .eq('followerid', session.id).eq('followingid', target.id)
