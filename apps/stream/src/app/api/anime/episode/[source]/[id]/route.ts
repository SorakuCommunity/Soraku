// src/app/api/anime/episode/[source]/[id]/route.ts
...
async function getSamehadakuStreams(episodeId: string) {
  try {
    // For WordPress-based sources, we need to fetch the episode page
    // and extract video links
    const res = await fetch(episodeId, { next: { revalidate: 60 } });
    if (!res.ok) return null;
    const html = await res.text();

    // Extract m3u8 links from player
...
export async function GET(
  ...
  switch (animeSource) {
    ...
    case "otakudesu":
    case "anibaru":
      // These use direct episode URLs as IDs
      const directStreams = await getSamehadakuStreams(id);
