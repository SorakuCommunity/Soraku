        ...
        <div className="flex justify-between mt-16">
          <div className="flex-1 max-w-[20%] items-center justify-end text-lg relative">
            <Combobox value={type} onChange={(e) => handleChange(e)}>
              <Combobox.Button className="h-full w-full gap-5 py-[2px] bg-secondary/70 rounded text-sm font-Archivo flex items-center justify-between px-2">
                {type.name}
                <ChevronDownIcon
          ...
          {data !== null
            ? data?.map((x, index) => (
                <div key={x.id + index} className="flex flex-col gap-2">
                  ...
                  <div>
                    <h1 className="line-clamp-2 font-Archivo font-bold">
                      {x.title}
                    </h1>
                  </div>
                </div>
