// api/v2/badges/developer endpoint
...
export default async function handler(req, res) {
    ...
    try {
        ...
        if (req.body.method === "add") {
            data.developers.push(req.body.username);
        } else if (req.body.method === "remove") {
            let index = data.developers.indexOf(req.body.username);
            if (index !== -1) { data.developers.splice(index, 1); }
        } else {
