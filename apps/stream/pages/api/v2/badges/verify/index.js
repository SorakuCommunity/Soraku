...
export default async function handler(req, res) {
    ...
    try {
        ...
        if (req.body.method === "add") {
            data.verifiedUsers.push(req.body.username);
        } else if (req.body.method === "remove") {
            let index = data.verifiedUsers.indexOf(req.body.username);
            if (index !== -1) { data.verifiedUsers.splice(index, 1); }
        } else {
