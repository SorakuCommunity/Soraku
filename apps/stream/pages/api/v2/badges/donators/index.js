// api/v2/badges/donators endpoint
...
export default async function handler(req, res) {
    ...
    try {
        ...
        if (req.body.method === "add") {
            data.donatorUsers.push(req.body.username);
        } else if (req.body.method === "remove") {
            let index = data.donatorUsers.indexOf(req.body.username);
            if (index !== -1) { data.donatorUsers.splice(index, 1); }
        } else {
