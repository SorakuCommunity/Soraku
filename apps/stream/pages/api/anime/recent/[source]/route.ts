// pages/api/anime/recent/[source]/route.ts
...
export async function GET(
  ...
  switch (animeSource) {
    ...
    default:
      ...
      results =
        anifyResults && anifyResults.length > 0
          ? anifyResults
          : (await getGogoRecent(page)) || [];
  }

  if (results.length === 0) results = (await getAnifyRecent(page)) || [];
  return NextResponse.json({ results, source: animeSource, page });
}
