  ...
  return (
    <div>
      <div
        ...
        onClick={() => router.push(`/hanime/trending`)}
      >
      ...
      <div className="relative flex items-center lg:gap-2">
        <div
          onClick={slideLeft}
          className={`flex items-center mb-5 cursor-pointer hover:text-action absolute left-0 bg-gradient-to-r from-[#100C11] z-40 h-full hover:opacity-100 ${
        ...
        >
          {data.map((item) => (
            <div key={item.id} className="flex flex-col gap-3 shrink-0 cursor-pointer">
              <Link legacyBehavior href={`/hanime/video/${item.id}`}>
        ...
        <MdChevronRight
          onClick={slideRight}
          size={30}
          className={`hidden md:block mb-5 cursor-pointer hover:text-action absolute right-0 bg-gradient-to-l from-[#100C11] z-40 h-full hover:opacity-100 hover:bg-gradient-to-l ${
