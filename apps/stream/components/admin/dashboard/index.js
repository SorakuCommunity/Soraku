...
export default function AdminDashboard({
  ...
  const [badgeType, setBadgeType] = useState("verify");
  const [badgeMethod, setBadgeMethod] = useState("add");
  ...
  const handleRemove = async () => {
    try {
      ...
      setBroadcast(prevState => ({ ...prevState, show: false }));
    } catch (error) {
      console.log(error);
    }
  };
  ...
  const handleBadgeSubmit = async (e) => {
    e.preventDefault();
    try {
      const res = await fetch(`/api/v2/badges/${badgeType}`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          "Authorization": `Bearer ${badgePassword}`,
        },
        body: JSON.stringify({
          method: badgeMethod,
  ...
  return (
    <div className="flex flex-col gap-5 px-5 py-10 h-full">
      <div className="flex flex-col gap-2">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-5">
          <div className="flex-center flex-col bg-secondary rounded p-5">
            <p className="font-Archivo text-4xl">{animeCount}</p>
            <p className="font-Archivo text-xl">Anime</p>
          </div>
          <div className="flex-center flex-col bg-secondary rounded p-5">
            <p className="font-Archivo text-4xl">{infoCount}</p>
            <p className="font-Archivo text-xl">Detail Info</p>
          </div>
          <div className="flex-center flex-col bg-secondary rounded p-5">
            <p className="font-Archivo text-4xl">{metaCount}</p>
            <p className="font-Archivo text-xl">Metadata</p>
          </div>
        </div>
      </div>
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-5 h-full">
        <div className="flex flex-col gap-2">
          ...
          <div className="flex flex-col justify-between bg-secondary rounded p-5 h-full">
            <form onSubmit={handleSubmit}>
              ...
              <div className="flex flex-col sm:flex-row font-Archivo font-semibold gap-2">
                <button
                  title="Broadcast"
                  type="submit"
                  className="bg-image text-white py-2 px-4 rounded-md hover:bg-opacity-80 transition duration-300 w-full sm:w-auto"
                >
                  Broadcast
                </button>
                <button
                  ...
                  className="bg-red-700 text-white py-2 px-4 rounded-md hover:bg-opacity-80 transition duration-300 w-full sm:w-auto"
                >
                  Remove
                </button>
              </div>
        ...
        <div className="flex flex-col gap-2">
          <p className="font-semibold">Manage User Badges</p>
          <div className="bg-secondary rounded p-5 h-full">
            <form onSubmit={handleBadgeSubmit} className="flex flex-col gap-4">
              <div>
                <label htmlFor="badgeType" className="block text-txt font-light mb-2">
                  Badge Type
                </label>
                <select
                  id="badgeType"
              ...
              <div>
                <label htmlFor="badgeMethod" className="block text-txt font-light mb-2">
                  Action
                </label>
                <select
                  id="badgeMethod"
                  value={badgeMethod}
                  onChange={(e) => setBadgeMethod(e.target.value)}
                  className="w-full px-3 py-2 border rounded-md focus:outline-none text-black"
                >
              ...
              <div>
                <label htmlFor="badgeUsername" className="block text-txt font-light mb-2">
                  Username
                </label>
                <input
                  type="text"
              ...
              <div>
                <label htmlFor="badgePassword" className="block text-txt font-light mb-2">
                  Password
                </label>
                <input
                  type="password"
