...
import "./components/ambientEffect.module.css"; 
import {
  MediaPlayer,
  MediaProvider,
  useMediaStore,
...
export default function VidStack({
  ...
  const [isAmbientMode, setIsAmbientMode] = useState(false); // State for ambient mode

  // Check localStorage for ambientMode setting on component mount
  useEffect(() => {
    const ambientModeSetting = localStorage.getItem("ambientMode");
    if (ambientModeSetting === "true") {
  ...
  const toggleAmbientMode = () => {
    ...
    localStorage.setItem("ambientMode", newAmbientMode.toString()); // Update localStorage
  };
  ...
  useEffect(() => {
    if (qualities.length > 0) {
      ...
      const preferredHeight = preferredQuality ? Number(preferredQuality) : null; // Convert to number

      const sourceQuality = preferredHeight 
        ? qualities.find(q => q.height === preferredHeight) // Match height with preferredQuality
        : qualities.reduce(
            (max, obj) => (obj.height > max.height ? obj : max),
            qualities[0]
          );

      // Check if sourceQuality is valid before calculating aspect ratio
      if (sourceQuality && sourceQuality.width && sourceQuality.height) {
  ...
  useEffect(() => {
    console.log("Track data:", track); // Log track data to help diagnose
    console.log("Default quality:", defaultQuality); // Log default quality details

    if (!defaultQuality || !defaultQuality.url) {
      console.warn("Warning: defaultQuality or its URL is undefined");
    }
  }, [track, defaultQuality]);
  ...
  useEffect(() => {
    ...
    function handlePause() {
      ...
      setIsPlaying(false);
      setIsIdle(true); // Set idle when paused
      setIdleTimeout(setTimeout(() => setIsIdle(true), 5000)); // Set idle after 5 seconds of idleness
    }

    function handleEnd() {
      // console.log("Player ended");
      setIsPlaying(false);
      setIsIdle(true); // Set idle when ended
      setIdleTimeout(setTimeout(() => setIsIdle(true), 5000)); // Set idle after 5 seconds of idleness
    }
    ...
    return () => {
      ...
      if (idleTimeout) {
        clearTimeout(idleTimeout); // Clear timeout on cleanup
      }
    };
  }, [id, duration]);
  ...
  useEffect(() => {
    ...
    return player.current!.subscribe(({ canPlay }: { canPlay: boolean }) => {
  ...
  // const videoSource = defaultQuality?.url
  //   ? `https://m3u8proxy.anifusion.workers.dev/?url=${defaultQuality.url}`
  //   : ""
  ...
  function onEnded() {
    ...
    if (autoNext === "on") {
      ...
      timeoutId = setTimeout(() => {
        ...
        if (navigation?.next) {
          router.push(
            `/anime/watch/${dataMedia.id}/${track.provider}?id=${
              navigation?.next?.id
  ...
  function onTimeUpdate(detail: MediaTimeUpdateEventDetail) {
    ...
    if (
      ...
      if (localStorage.getItem("autoskip") === "on") {
        remote.seek(op.endTime); // Auto skip opening
        toast.success("Skipped Opening!"); // Show toast notification
      }
    } else {
    ...
    if (
      ...
      if (localStorage.getItem("autoskip") === "on") {
        remote.seek(ed.endTime); // Auto skip ending
        toast.success("Skipped Ending!"); // Show toast notification
      }
    } else {
  ...
  return (
      <MediaPlayer
        key={id}
        className={`${style.player} player relative ${isAmbientMode ? 'ambient' : ''}`} // Add ambient class
        title={
          navigation?.playing?.title ||
