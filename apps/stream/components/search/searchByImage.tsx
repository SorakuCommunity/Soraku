...
type SearchByImageProps = {
  ...
  setData?: (data: TraceMoeResultTypes[]) => void;
...
export default function SearchByImage({
  ...
  async function findImage(formData: FormData) {
    ...
    response
      .then((data: unknown) => {
        const typedData = data as TraceMoeDataTypes;
        if (typedData && typedData.result?.length > 0) {
          const id = typedData.result[0].anilist.id;
          const filteredData = typedData.result.filter(
            (i: TraceMoeResultTypes) => i.anilist.isAdult === false
          );
          if (setData) setData(filteredData);
          if (searchPalette) router.push(`/anime/${id}`);
          if (setIsOpen) setIsOpen(false);
          if (setMedia) setMedia(null); // Set to null instead of an empty object
        }
      })
      .catch((error) => {
