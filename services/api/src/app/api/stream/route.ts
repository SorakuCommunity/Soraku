...
export async function GET(req: NextRequest) {
  ...
  if (searchParams.get("anime") === "true") {
    ...
    if (searchParams.get("sources") === "true") {
      const sources = await withCache("anime:sources", TTL.SOURCES, getAvailableSources)
    ...
    const results  = await withCache(cacheKey, TTL.SEARCH, () => searchAnime(q, source, page))
