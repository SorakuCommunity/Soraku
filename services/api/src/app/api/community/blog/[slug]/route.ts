...
export async function GET(
  ...
  const comments = await db
    ...
    .where(
      and(
        eq(post_comments.postid, post.id),
        eq(post_comments.isdeleted, false),
      ),
    )
    .orderBy(asc(post_comments.createdat));

  const userIds = [
    ...new Set(comments.filter((c) => c.userid).map((c) => c.userid!)),
  ];
  let authorsMap: Record<string, unknown> = {};
  if (userIds.length > 0) {
...
export async function POST(
  ...
  if (!userId && !parsed.data.guestname?.trim()) {
    return NextResponse.json(
      { data: null, error: "Nama wajib diisi untuk komentar tamu." },
      { status: 400 },
    );
  }
