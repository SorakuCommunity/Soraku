...
async function handleTicketCreate(interaction, client, panelId, categoryIndex) {
  ...
  const existingTickets = await db.getUserTickets(
  ...
  const ticketId = await db.incrementTicketCounter(interaction.guild.id);
...
async function handleTicketClaim(interaction, client, channelId) {
  const ticketData = await db.getTicket(channelId);
  ...
  const panel = await db.getTicketPanel(interaction.guild.id, ticketData.panel_id);
  if (!panel) {
    return interaction.reply({
...
async function handleTicketClose(interaction, client, channelId) {
  ...
  try {
    ...
    const alreadySentTranscript = await db.isTranscriptSent(channelId);
    ...
    if (!alreadySentTranscript && panel.transcriptChannel) {
      ...
      if (transcriptChannel) {
        ...
        await db.markTranscriptSent(channelId);
      }
    }

    await db.closeTicket(channelId, interaction.user.id);
    ...
    const alreadySentReview = await db.isReviewSent(channelId);
...
async function handleTicketRate(interaction, client, channelId, rating) {
  ...
  if (ticketData) {
    const panel = await db.getTicketPanel(ticketData.guild_id, ticketData.panel_id);
...
async function handleReviewModalSubmit(interaction, client, channelId) {
  ...
  if (ticketData) {
    await db.rateTicket(channelId, ticketData.rating || 0, reviewComment);
  }
  ...
  if (ticketData && ticketData.guild_id) {
    ...
    if (guild) {
      panel = await db.getTicketPanel(ticketData.guild_id, ticketData.panel_id);
    }
  }

  if (!guild && ticketData) {
    for (const [, g] of client.guilds.cache) {
      const p = await db.getTicketPanel(g.id, ticketData.panel_id);
