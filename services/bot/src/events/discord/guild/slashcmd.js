...
async function _sendCooldownError(interaction, cooldownTime, command) {
  ...
  const hasPremium = await db.hasAnyPremium(interaction.user.id, interaction.guild.id);
...
async function handleChatInputCommand(interaction, client) {
  ...
  try {
    ...
    if (
      await db.isUserBlacklisted(interaction.user.id) ||
      await db.isGuildBlacklisted(interaction.guild.id)
    ) {
      return _sendError(
        interaction,
        "Access Denied",
