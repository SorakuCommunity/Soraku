...
async function _sendCooldownError(message, cooldownTime, command) {
  ...
  const hasPremium = await db.hasAnyPremium(message.author.id, message.guild.id);
...
async function _handleExpiredUserPerks(userId, author) {
  const hasNoPrefix = await db.hasNoPrefix(userId);
  const userPrefixes = await db.getUserPrefixes(userId);
  if (!hasNoPrefix && userPrefixes.length === 0) return;

  if (!await db.isUserPremium(userId)) {
    let perksRemoved = [];
    if (hasNoPrefix) {
      await db.setNoPrefix(userId, false, null);
      perksRemoved.push("No-Prefix Mode");
    }
    if (userPrefixes.length > 0) {
      await db.setUserPrefixes(userId, []);
...
async function _handleExpiredGuildPerks(guildId, channel) {
  if (await db.isGuildPremium(guildId)) return;
  const prefixes = await db.getPrefixes(guildId);
  if (prefixes.length > 1) {
    await db.setPrefixes(guildId, [config.prefix]);
...
function _parseCommand(message, client) {
  ...
  if (mentionMatch) {
    ...
    if (await db.isUserPremium(message.author.id)) {
      const userPrefix = db
...
export default {
  ...
  async execute(message, client) {
    ...
    if (
      await db.isUserBlacklisted(message.author.id) ||
      await db.isGuildBlacklisted(message.guild.id)
    )
      return;
    ...
    if (mentionRegex.test(message.content.trim())) {
      ...
      const guildPrefixes = await db.getPrefixes(message.guild.id);
    ...
    try {
      ...
      if (command.userPrem && !await db.isUserPremium(message.author.id))
        return _sendPremiumError(message, "user");
      if (command.guildPrem && !await db.isGuildPremium(message.guild.id))
        return _sendPremiumError(message, "guild");
      if (
        command.anyPrem &&
        !await db.hasAnyPremium(message.author.id, message.guild.id)
      )
        return _sendPremiumError(message, "user");
