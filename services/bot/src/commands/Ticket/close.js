...
export default {
  ...
  async execute({ client, message, args }) {
    ...
    try {
      const alreadySentTranscript = await db.isTranscriptSent(message.channel.id);
      ...
      if (!alreadySentTranscript && panel.transcriptChannel) {
        ...
        if (transcriptChannel) {
          ...
          await db.markTranscriptSent(message.channel.id);
        }
      }

      await db.closeTicket(message.channel.id, message.author.id);
      ...
      const alreadySentReview = await db.isReviewSent(message.channel.id);
