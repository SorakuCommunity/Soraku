...
export default {
  ...
  async execute({ client, message, args, prefix }) {
    ...
    const panelId = await db.incrementPanelCounter(message.guild.id);
    ...
    try {
      ...
      await db.createTicketPanel(setupData);
      ...
      await db.updateTicketPanel(message.guild.id, panelId, {
      ...
      if (error.message === "time" || error.size === 0) {
        await db.decrementPanelCounter(message.guild.id);
        return sendError(
          message,
