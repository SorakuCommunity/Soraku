...
class PrefixCommand extends Command {
  ...
  async _handleCommand(ctx, newPrefix) {
    const isPremium = await db.isGuildPremium(ctx.guild.id);
  ...
  async _setPrefix(ctx, newPrefix, isPremium) {
    ...
    let prefixes = await db.getPrefixes(ctx.guild.id);
    let updateMessage;
    ...
    await db.setPrefixes(ctx.guild.id, prefixes);
  ...
  async _showManagementUI(ctx, isPremium) {
    const prefixes = await db.getPrefixes(ctx.guild.id);
    ...
    collector.on("collect", async (interaction) => {
      try {
        if (interaction.customId === "prefix_add") {
          ...
          let prefixes = await db.getPrefixes(guildId);
          ...
          prefixes.push(newPrefix);
          await db.setPrefixes(guildId, prefixes);
          ...
          await db.setPrefixes(guildId, defaultPrefixes);
