...
export default {
  ...
  async execute({ client, message, args }) {
    ...
    try {
      await db.addWarn(message.guild.id, target.id, message.author.id, reason);
      const warnCount = await db.getWarnCount(message.guild.id, target.id);
  ...
  async slashExecute({ client, interaction }) {
    ...
    try {
      await db.addWarn(interaction.guild.id, target.id, interaction.user.id, reason);
      const warnCount = await db.getWarnCount(interaction.guild.id, target.id);
