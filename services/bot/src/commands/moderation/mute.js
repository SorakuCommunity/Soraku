...
export default {
  ...
  async execute({ client, message, args }) {
    ...
    try {
      ...
      await db.addMute(message.guild.id, target.id, message.author.id, reason, durationMs);
  ...
  async slashExecute({ client, interaction }) {
    ...
    try {
      await target.timeout(durationMs || 28 * 24 * 60 * 60 * 1000, reason);
      await db.addMute(interaction.guild.id, target.id, interaction.user.id, reason, durationMs);
