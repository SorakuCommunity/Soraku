...
class InviterCommand extends Command {
  ...
  async _getInviter(client, guild, user) {
    const trackingEnabled = await db.isInviteTrackingEnabled(guild.id);
    
    if (!trackingEnabled) {
      return this._createErrorContainer("Invite tracking is disabled in this server.");
    }

    const inviteData = await db.getMemberInvites(guild.id, user.id);
    ...
    const inviterData = await db.getMemberInvites(guild.id, inviteData.inviter_id);
    const inviterEffective = await db.getEffectiveInvites(inviterData);
