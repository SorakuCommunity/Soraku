...
class ResetInvitesCommand extends Command {
  ...
  async _resetInvites(guild, user) {
    await db.resetMemberInvites(guild.id, user.id);
    const memberData = await db.getMemberInvites(guild.id, user.id);
    const effectiveInvites = await db.getEffectiveInvites(memberData);
