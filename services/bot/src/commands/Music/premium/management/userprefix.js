...
class UserPrefixCommand extends Command {
  ...
  async _addPrefix(userId, newPrefix) {
    ...
    const currentPrefixes = await db.getUserPrefixes(userId);
    ...
    await db.setUserPrefixes(userId, newPrefixes);
  ...
  async _handleCommand(ctx, directPrefix = null) {
    ...
    const message = await (isInteraction
      ? ctx.reply({
          components: [this._buildUIManagementContainer(username, await db.getUserPrefixes(userId))],
          flags: MessageFlags.IsComponentsV2,
          fetchReply: true,
        })
      : ctx.channel.send({
  ...
  _setupCollector(message, userId, username) {
    ...
    collector.on("collect", async (interaction) => {
      try {
        let prefixes = await db.getUserPrefixes(userId);
        ...
        if (interaction.customId === "up_add") {
          ...
          try {
            ...
            prefixes = await db.getUserPrefixes(userId);
          ...
          await db.setUserPrefixes(userId, prefixes);
