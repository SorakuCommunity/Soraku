...
class BlacklistCommand extends Command {
	...
	async _handleAdd(client, message, args) {
		...
		try {
			...
			if (type === "user") {
				alreadyBlacklisted = await db.isUserBlacklisted(id);
				if (!alreadyBlacklisted) {
					result = await db.blacklistUser(id, reason);
				}
			} else {
				alreadyBlacklisted = await db.isGuildBlacklisted(id);
				if (!alreadyBlacklisted) {
					result = await db.blacklistGuild(id, reason);
				}
			}
	...
	async _handleRemove(client, message, args) {
		...
		try {
			const isUserBlacklisted = await db.isUserBlacklisted(id);
			const isGuildBlacklisted = await db.isGuildBlacklisted(id);
			...
			if (isUserBlacklisted) {
				result = await db.unblacklistUser(id);
				type = "user";
			} else {
				result = await db.unblacklistGuild(id);
				type = "guild";
			}
