...
export class Database {
  ...
  async upsert(table, data, conflict) {
    const { data: r, error } = await bot().from(table).upsert(data, { onConflict: conflict }).select().maybeSingle()
    if (error) { logger.error('DB', `upsert ${table}`, error); throw error }
    return r
  }
  ...
  async findOne(table, filters = {}) {
    ...
    if (error) { logger.error('DB', `findOne ${table}`, error); throw error }
    return data
  }
  ...
  async findAll(table, filters = {}) {
    ...
    if (error) { logger.error('DB', `findAll ${table}`, error); throw error }
    return data ?? []
  }
  ...
  async destroy(table, filters = {}) {
    ...
    for (const [k, v] of Object.entries(filters)) q = q.eq(k, v)
    const { error } = await q
    if (error) { logger.error('DB', `destroy ${table}`, error); throw error }
  }
  ...
  async update(table, data, filters = {}) {
    ...
    for (const [k, v] of Object.entries(filters)) q = q.eq(k, v)
    const { error } = await q
    if (error) { logger.error('DB', `update ${table}`, error); throw error }
  }
