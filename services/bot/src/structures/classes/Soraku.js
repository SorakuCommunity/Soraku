...
export class Soraku extends Client {
  ...
  async init() {
    ...
    try {
      ...
      this.db.user
        .findAll("users", { no_prefix: true })
        .then((rows) => {
          rows.forEach((u) => this.noPrefixUsers.add(u.id));
          this.logger.info(
            "Soraku",
            `Loaded ${this.noPrefixUsers.size} no-prefix users`,
          );
        })
        .catch(() => {});

      // TODO: Uncomment setelah deploy ke production dgn Interactions Endpoint URL
  ...
  async cleanup() {
    ...
    try {
      ...
      this.destroy();
      this.logger.success("Soraku", "❄️ Cleanup completed successfully. 🌸");
    } catch (error) {
      this.logger.error(
        "Soraku",
