 ...
 * Baca file .sql di database/migrations/, jalankan yang belum dijalankan.
...
if (!SUPABASE_URL || !SERVICE_KEY) {
  console.error(
    "❌ ENV tidak lengkap: butuh SUPABASE_URL + SUPABASE_SERVICE_KEY",
  );
  process.exit(1);
}
...
if (!PROJECT_REF) {
  console.error("❌ Tidak bisa parse project ref dari URL");
  process.exit(1);
}
...
async function main(): Promise<void> {
  ...
  const migrDir = join(root, "database", "migrations");
  ...
  if (!files.length) {
    console.log("ℹ  Tidak ada file .sql");
    return;
  }
  ...
  console.log(
    `📋 ${files.length} file migration | ${applied.size} sudah applied\n`,
  );
